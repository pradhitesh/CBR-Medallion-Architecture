...
def return_tables(schema_type: Literal['cohort', 'shared', 'orphan'], schema_name: str=None) -> ReturnedTables:
    # Fresh DeclarativeBase per call: each invocation gets its OWN MetaData,
    # so calling return_tables('cohort_101_data') and return_tables('cohort_102_data')
    # in the same process produces two independent metadatas with no table-key
    # collisions. Cross-schema FKs to `shared.*` are string references, so they
    # don't need the shared MetaData to be present here.
    ...
    class RedMeasurement(Measurement):
    ...
    class SourceLog(Base):
        ...
        __table_args__ = (
            ForeignKeyConstraint(['instrument_cat'], ['shared.instrument_catalogue.id']),
            ForeignKeyConstraint(['api_catalogue_id'], ['shared.api_catalogue.id']),
            PrimaryKeyConstraint('id'),
            {'schema': schema_name}
        )
